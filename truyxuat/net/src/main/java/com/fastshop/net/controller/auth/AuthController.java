    ...
    public String authLogin(@Validated @ModelAttribute("dtoLogin") DtoLogin dtoLogin, Errors errors) {
        if (errors.hasErrors()) {
            ...
            try {
                ...
                return "redirect:%s".formatted( auth.getRole().getId().equals("ADMIN") ? "/admin/home" : (auth.getRole().getId().equals("STAFF") ? "/staff/home" : (auth.getRole().getId().equals("STAFF_M") ? "/staff/home" : (auth.getRole().getId().equals("STAFF_S") ? "/staff/home" : "/"))));
        ...
        if (errors.hasErrors()) {
            ...
            try {
                ...
                if (auth.getRole().getId().equals("USER")) {
                    ...
                    return auth.getRole().getId().equals("ADMIN") ? "redirect:/admin/home" : (auth.getRole().getId().equals("STAFF") ? "redirect:/staff/home" : (auth.getRole().getId().equals("STAFF_M") ? "redirect:/staff/home" : (auth.getRole().getId().equals("STAFF_S") ? "redirect:/staff/home" : "/")));
                }
            } catch (Exception e) {
                return "error/error-404";
            }
