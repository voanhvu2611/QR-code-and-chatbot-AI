...
public class StaffController {
    ...
    public String dicsount(Model model, @ModelAttribute("auth") Authority auth) {
        try {
            ...
            model.addAttribute("count_notify", notifyService.findAllByAccAndNowAndStatusTrueOrderBy(auth.getAccount()));
            model.addAttribute("title_main", "Nông sản Việt Giảm giá sản phẩm");
            return "index";
        } catch (Exception e) {
            e.printStackTrace();
            return "redirect:/login.nongsanviet.com";
    ...
    public String status(Model model, @ModelAttribute("auth") Authority auth, @PathVariable("id") Integer status) {
        try {
            model.addAttribute("mailInfo", new MailInfo(auth.getAccount().getEmail(), "Thông báo đơn hàng của bạn đã bị hủy từ nongsanviet.com", ""));
