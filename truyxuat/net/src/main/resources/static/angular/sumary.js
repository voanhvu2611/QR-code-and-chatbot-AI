...
app.controller("checkout-ctrl", ($scope, $http) => {
    ...
    $scope.cart = {
        ...
        get amountDeliver() {
            var tongtien = Number(this.amount + 12000);
        ...
        if ($('#btn-free').prop('disabled', false)) {
            setTimeout(() => {
                $http.get(path).then((resp) => {
                    ...
                    if (!$scope.discount_add_voucher) {
                    ...
                    else if ($scope.discount_add_voucher && ($scope.discount_add_voucher.number <= 0) ) {
                        $("#error_find").css("opacity", "1");
                        $("#error_find").text("(*) Mã giảm giá đã hết lượt sử dụng.");
                        $("#success_find").css("opacity", "0");
                        $("#btn-free").attr("disabled", "disabled");
                    }
                        ...
                        localStorage.setItem("id_discount", $scope.discount_add_voucher.id);
                    }
                }).catch((err) => { 
                    console.log(err);
                });
    ...
    $scope.order = {
        thread1(){
            const selectedPayment = JSON.parse(localStorage.getItem("payment"));
            if (selectedPayment === "bill-2") {
                ...
                $http.post(url, $scope.form).then((resp) => {
                    ...
                    const data = {
                        totalPayment: $scope.orderNow.total,
                        orderIdsString: $scope.orderNow.id
                    };

                    const apiUrl = `${host_}/vnpay/payment`;
                    const vnpayUrl = apiUrl + `?totalPayment=${data.totalPayment}&orderIdsString=${data.orderIdsString}`;
                    ...
                    $http.post(vnpayUrl)
                        .then((response) => {
                            ...
                            if (localStorage.getItem("id_discount")) {
                                const update_discount = `${host_}/discounts/update/number/${localStorage.getItem("id_discount")}`;
                                $http.put(update_discount).then((resp) => {
        ...
        thread3() {
            ...
            Swal.fire({
                title: 'Thành công!',
                text: 'Đặt hàng thành công. Cảm ơn bạn đã mua hàng tại Nông sản Việt.',
                icon: 'success',
                confirmButtonText: 'OK'
            }).then(() => {
                var url_get = `${host_}/orders/update-products/${$scope.orderNow.id}`;
                $http.put(url_get).then((resp) => {
                    console.log("update ok", resp);
                }).catch((err) => {
                    console.log("update failed", err);
                });
                setTimeout(() => {
                    window.location.href = "http://localhost:8080/nongsanviet.com";
...
app.controller("product-ctrl", ($scope, $http) => {
    ...
    $scope.viewShipping = (productId) => {
        var url = `${host_}/products/shipping/${productId}`;
        $http.get(url).then((resp) => {
            $scope.shipping = resp.data;  // Lưu data vào $scope.shipping
            $scope.shipping.importDate = new Date($scope.shipping.importDate);
            console.log("Shipping info loaded", resp);
        }).catch((err) => {
            console.log("Error loading shipping info", err);
        });
    };

    // Hàm để load thông tin manufacturing khi click vào nút xem
    $scope.viewManufacturing = (productId) => {
        console.log(productId);
        var url = `${host_}/products/manufacturing/product/${productId}`;
        console.log(url);
        $http.get(url).then(resp => {
            $scope.manufacturing = resp.data;
            // Chuyển đổi ngày từ string sang Date object
            if($scope.manufacturing.manufacturingDate) {
                $scope.manufacturing.manufacturingDate = new Date($scope.manufacturing.manufacturingDate);
            }
        }).catch(error => {
    ...
    $scope.viewAllInfo = (productId) => {
        $scope.edit(productId);
        $scope.viewShipping(productId);
        $scope.viewManufacturing(productId);
    }

    // Hàm để cập nhật thông tin manufacturing
    $scope.updateManufacturing = () => {
        var item = angular.copy($scope.manufacturing);
        var url = `${host_}/products/manufacturing/${$scope.manufacturing.id}`;
    ...
    $scope.resetManufacturingUpdated = () => {
        Swal.fire({
            title: 'Bạn có chắc chắn?',
            text: "Dữ liệu sẽ bị thay đổi.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                var url = `${host_}/products/manufacturing/reset/${$scope.manufacturing.id}`;
                $http.put(url).then(resp => {
                    Swal.fire({
                        title: 'Thành công!',
                        text: 'Đặt lại thông tin sản phẩm thành công.',
                        icon: 'success',
                        confirmButtonText: 'OK'
                    });
                    setTimeout(() => {
    ...
    $scope.resetShippingUpdated = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = `${host_}/products/reset/shipping/${$scope.shipping.id}`;
                $http.put(url).then(resp => {
                    Swal.fire({
                        title: 'Thành công!',
                        text: 'Đặt lại thông tin vận chuyển thành công.',
                        icon: 'success',
                        confirmButtonText: 'OK'
                    });
                    setTimeout(() => {
    ...
    $scope.updateShipping = () => {
        console.log("Updating shipping", $scope.shipping.id);
        
        var url = `${host_}/products/shipping/${$scope.shipping.id}`;
        var data = angular.copy($scope.shipping);
        
        $http.put(url, data).then((resp) => {
            console.log("Shipping updated", resp);
            Swal.fire({
                title: 'Thành công!',
                text: 'Cập nhật thông tin vận chuyển thành công.',
                icon: 'success',
                confirmButtonText: 'OK'
            });
            setTimeout(() => {
            ...
            console.log("Error updating shipping", err);
            Swal.fire({
                title: 'Thất bại!',
                text: 'Cập nhật thông tin vận chuyển thất bại.',
                icon: 'error',
                confirmButtonText: 'OK'
            });
            setTimeout(() => {
    ...
    $scope.update = () => {
        ...
        formData.append('category', $scope.form.category.id);
        formData.append('price', $scope.form.price);
        formData.append('number', $scope.form.number);
        ...
        // Thêm tên ảnh hiện tại (nếu không có file mới)
        formData.append('imageName', $scope.form.image);
    ...
    $scope.delete = (id) => {
        Swal.fire({
            title: 'Bạn có chắc chắn?',
            text: "Sản phẩm sẽ không thể khôi phục sau khi xóa!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#3085d6',
            ...
            if (result.isConfirmed) {
                ...
                $http.delete(url)
                .then((resp) => {
                    console.log("Product deleted successfully:", resp);
                    window.location.href = "http://localhost:8080/staff/products";
                })
                .catch((err) => {
